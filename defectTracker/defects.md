1. Chrome / Firefox : drawing area gets resized after first data change. Seems to be a problem with the browser size calculation timing - seems to assume it needs a vertical page scoll bar at initial sizing. (no problem in IE)
2. (related to 1.) brush extent cannot be moved to very right border.
3. Charts so not resize when browser window resizes (not related to 1 and 2)
...
17. Brush (All charts) While brushing, moving mouse over legend or outside drawing area auses HTML text selection highlighting.
